...
class TdExample {
private:
    std::string telephone = "[redacted data]";
    ...
    void loop() {
        while (true) {
            if (need_restart_) {
                ...
                std::cout
                        << "Enter action [q] quit [u] check for updates and request results [c] show chats [m <chat_id> "
                ...
                if (action == "u") {
                    ...
                    std::cout << "Downloading added chats..." << std::endl;
                    stop_dump = false;
                    //std::cerr << "get updates for purposes of to prosratsya:";
                    //while (true) {
                    //	auto response = client_manager_->receive(0);
                    //	if (response.object) {
                    //		process_response(std::move(response));
                    ...
                    // big saver to file
                    auto handle_dump_chat = [this](Object object) {
                        if (object->get_id() == td_api::error::ID) {
                            std::cerr << "Error. Obj: " << to_string(object) << std::endl;
                            stop_dump = true;
                            return;
                        }
                        auto events = td::move_tl_object_as<td_api::chatEvents>(object);
                        if (events->events_.empty()) {
                            // похоже, пора!
                            std::cerr << "total processed entries: " << processed_entries_count;
                            std::ofstream of(this->json_filename);
                            of << entries;
                            stop_dump = true;
                        }
                        for (auto &event: events->events_) {
                            processed_entries_count++;
                            if (event->user_id_ != this->admin_log_admin_action_id) {
                                continue;
                            }
                            std::cerr << (dumped_count + 1) << ". " << "Chat Event of user " << event->user_id_
                                      << ", date: " << date_printer(event->date_) << std::endl;
                            if (event->action_->get_id() != td_api::chatEventMessageDeleted::ID) {
                                std::cerr << "(NOT a deleted message, skipping)";
                                continue;
                            }
                            auto action = td_api::move_object_as<td_api::chatEventMessageDeleted>(event->action_);
                            std::cerr << "original message date: " << date_printer(action->message_->date_);
                            nlohmann::json entry;
                            //entry["deleted_date"] = event->date_;
                            entry["date"] = action->message_->date_;


                            td_api::downcast_call(*action->message_->content_, overloaded(
                                                          [&, this](td_api::messageText &text) {
                                                              entry["text"] = text.text_->text_;
                                                              std::cerr << "Text: " << text.text_->text_;
                                                              if (text.text_->entities_.size() != 0) {
                                                                  std::cerr << ", entities: [";
                                                                  entry["entities"] = nlohmann::json::array({});
                                                              }
                                                              for (const auto &ent: text.text_->entities_) {
                                                                  nlohmann::json entity;
                                                                  auto type = to_string(ent->type_);
                                                                  /*entity["type"] = type;
                                                                  entity["offset"] = ent->offset_;
                                                                  entity["l"]*/
                                                                  std::cerr << to_string(ent);
                                                                  //std::cerr << "{Type: " << type << ", offset: " << ent->offset_
                                                                  // << ", length: " << ent->length_ << "},\n";

                                                              }
                                                              std::cerr << "]";
                                                          },
                                                          [&, this](td_api::messageChatJoinByLink &msg) {
                                                              entry["type"] = "join_by_link";
                                                          },
                                                          [](auto &val) { std::cerr << "another: " << to_string(val) << std::endl; }
                                                  )
                            );
...
//						std::cout << "Dumped json: " << entries;
                    };

                    while (!stop_dump) {
                        if (this->handled_query_id >= this->current_query_id_) {
                            std::cerr << "Dumping chat..." << processed_entries_count << "entries processed, "
                                      << dumped_count << " dumped" << std::endl;
                            auto filters = td_api::make_object<td_api::chatEventLogFilters>(/*message edits:*/false,
                                    /*message deletions: */ true,
                                    /*message pins: */false,/*member joins: */ false, false,
                                    false, false, false,
                                    /*info changes: */false, false, false, false);
                            std::vector<int32_t> user_ids = {};
                            send_query(td_api::make_object<td_api::getChatEventLog>(this->admin_log_chat_id,
                                                                                    "s",/*event id*/
                        ...
                        while (true) {
                            ...
                            auto response = client_manager_->receive(7.0);
                            if (response.object) {
                                process_response(std::move(response));
                            } else {
                                break;
                        ...
                        std::cerr << "Got and handled " << counter << "updates\n";
        ...
        td_api::downcast_call(
                *update, overloaded(
                        ...
                        [this](td_api::updateNewMessage &update_new_message) {
                            ...
                            std::cout << "Got message: [chat_id:" << chat_id << "] [from:" << sender_name << "] ["
                                      << text
                                      << "]" << std::endl;
                        },
                        [](auto &update) {}));
                ...
                *authorization_state_,
                overloaded(
                        ...
                        [this](td_api::authorizationStateWaitPhoneNumber &) {
                            ...
                            std::string phone_number = this->telephone;
